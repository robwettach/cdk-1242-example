...
Thanks Elad for showing me how to [access the CloudFormation layer](https://awslabs.github.io/aws-cdk/aws-construct-lib.html#access-the-aws-cloudformation-layer) for taking advantage of CloudFormation properties that aren't exposed via CDK.

I've run into an error using the Java bindings.  While attempting to [Override Resource Properties](https://awslabs.github.io/aws-cdk/aws-construct-lib.html#overriding-resource-properties) using the `getPropertyOverrides()` method I ran into a "Maximum stack size exceeded" error.  Using the `addOverride("Properties.RawMessageDelivery")` method succeeds.

Here's the stack trace.  You can get this as well by just calling `mvn package && cdk synth` in the `java` directory.  I also created this same stack in Typescript and did not experience this error.
